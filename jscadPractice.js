...
const createCounterSink = (width, dp, thk, counterSinkAEn, counterSinkBEn, counterSinkCEn) => {
  const createCSF = () => {
    ...
    return union(head, hole);
  };

  const calculatePositions = (totalLength) => {
    const gap = 30;
    const minDist = 100;
    ...
    return Array.from({ length: numScrews }, (_, i) => gap + i * interval);
  };
  ...
  if (counterSinkAEn) {
    const positions = calculatePositions(dp);
  ...
  if (counterSinkBEn) {
    const positions = calculatePositions(width);
    positions.forEach(position => {
